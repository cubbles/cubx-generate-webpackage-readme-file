...
(function () {
   ...
   */
  var WebpackageReadmeGenerator = function (webpackagePath) {
    if (!webpackagePath) {
  ...
  WebpackageReadmeGenerator.prototype.generateReadmeFile = function () {
    ...
    for (var artifactType in this.manifest.artifacts) {
      for (var i = 0; i < this.manifest.artifacts[artifactType].length; i++) {
        var artifact = this.manifest.artifacts[artifactType][i];
    ...
    var templatePath = path.join(__dirname, '../template/template.md');
    var dist = path.resolve(this._webpackagePath, 'README.md');
